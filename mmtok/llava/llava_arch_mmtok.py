...
# ------------------------------------------------------------------------
# Modified from VisionZip (https://github.com/dvlabresearch/VisionZip)
# Copyright 2025 Zoom Communications, Inc. / Author: Sixun Dong
# ------------------------------------------------------------------------
...
def encode_images_mmtok(self, images):
    image_features, keep_idx = self.get_model().get_vision_tower()(images)
    # image_features = self.get_model().mm_projector(image_features)
    return image_features


def encode_images_mmtok_multi(self, images):
    image_features, keep_idx = self.get_model().get_vision_tower()(images)
    return image_features, keep_idx


# Adapted from VisionZip / LLaVA-NeXT (LLaVA 1.5); modified for MMTok.
def restore_image_features_sorted(self, image_feature, cur_keep_idx, width, height):
    ...
    cur_keep_idx_sorted = cur_keep_idx  # [num_img, num_keep]
    cur_keep_idx_sorted_restore = cur_keep_idx_sorted
...
# Adapted from VisionZip / LLaVA-NeXT (LLaVA 1.5); modified for MMTok.
def prepare_inputs_labels_for_multimodal_mmtok(
    self,
    input_ids,
    position_ids,
    attention_mask,
    ...
    if type(images) is list or images.ndim == 5:
        ...
        image_features, keep_idxs = self.encode_images_mmtok_multi(concat_images)
        ...
        image_features = self.encode_images_mmtok(images)
